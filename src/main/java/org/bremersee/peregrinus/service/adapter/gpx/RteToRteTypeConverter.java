...
class RteToRteTypeConverter extends AbstractFeatureConverter {
  ...
  Tuple2<RteType, List<WptType>> convert(
    ...
    final String rtePtBaseName = getRtePtBaseName(rte.getProperties().getName(), usedNames);
  ...
  private int getEachRtePointValue(GpxExportSettings exportSettings) {
  ...
  private String getRtePtBaseName(final String rteName, final Set<String> usedNames) {
    final String name = rteName.length() > 12 ? rteName.substring(0, 12) : rteName;
    int counter = 0;
    String newName = name;
  ...
  private ExtensionsType getViaPoint(final GpxExportSettings exportSettings) {
